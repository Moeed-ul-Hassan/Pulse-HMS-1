...
class PatientManager:
    ...
    def new_patient_form(self):
        ...
        with st.form("new_patient_form"):
            ...
            medications = []
            num_meds = st.number_input("Number of medications", min_value=0, max_value=10, value=1)
            
            for i in range(num_meds):
                col1, col2, col3 = st.columns(3)
                with col1:
                    med_name = st.text_input(f"Medicine {i+1}", key=f"med_name_{i}")
                with col2:
    ...
    def existing_patient_visit_form(self):
        ...
        if patient_phone and patient_phone in self.patients["patients"]:
            ...
            with st.form("existing_patient_visit"):
                ...
                st.markdown("### 💊 Medications")
                medications = []
                num_meds = st.number_input("Number of medications", min_value=0, max_value=10, value=1, key="existing_num_meds")
                
                for i in range(num_meds):
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        med_name = st.text_input(f"Medicine {i+1}", key=f"existing_med_name_{i}")
                    with col2:
                        dosage = st.text_input(f"Dosage {i+1}", key=f"existing_dosage_{i}")
                    with col3:
                        duration = st.text_input(f"Duration {i+1}", key=f"existing_duration_{i}")
    ...
    def generate_patient_report(self, patient_data: Dict, visit_data: Dict) -> bytes:
        ...
        if visit_data["medications"]:
            ...
            for med in visit_data["medications"]:
                pdf.cell(0, 8, f'• {med["name"]} - {med["dosage"]} for {med["duration"]}', 0, 1)
        ...
        pdf.cell(0, 10, 'Crafted by Moeed ul Hassan @The legend', 0, 1, 'C')
